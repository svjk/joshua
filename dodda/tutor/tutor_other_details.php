...
<?php
	...
	{	
		...
		$phone_pattern = "/[2-9]{2}\d{8}/";
		
		//email and mobile number filtering for question1
		if (preg_match($email_pattern, $question_1_answer, $email_id)) 
		{
			$question_1_answer = str_replace($email_id[0], "", $question_1_answer);
		}		
		
		if (preg_match($phone_pattern, $question_1_answer, $phone)) 
		{
			$question_1_answer = str_replace($phone[0], "", $question_1_answer);
		}		
		
		//email and mobile number filtering for question2
		if (preg_match($email_pattern, $question_2_answer, $email_id)) 
		{
			$question_2_answer = str_replace($email_id[0], "", $question_2_answer);
		}		
		
		if (preg_match($phone_pattern, $question_2_answer, $phone)) 
		{
			$question_2_answer = str_replace($phone[0], "", $question_2_answer);
		}	
		
		//email and mobile number filtering for question3
		if (preg_match($email_pattern, $question_3_answer, $email_id)) 
		{
			$question_3_answer = str_replace($email_id[0], "", $question_3_answer);
		}		
		
		if (preg_match($phone_pattern, $question_3_answer, $phone)) 
		{
			$question_3_answer = str_replace($phone[0], "", $question_3_answer);
		}
